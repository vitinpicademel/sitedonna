...
export async function GET(req: Request) {
  ...
  // Se não encontrou nada, retorna array vazio ao invés de erro
  if (lastStatus === 404 || lastStatus === 500) {
    if (url.searchParams.get('debug')) {
      return NextResponse.json({ attempts, lastStatus, last: lastBody, fallback: "empty_array" }, { status: 200 })
    }
...
export async function POST(req: Request) {
  ...
  if (lastStatus === 404 || lastStatus === 500) {
    const debug = new URL(req.url).searchParams.get('debug')
    if (debug) {
      return NextResponse.json({ attempts, lastStatus, last: lastJson ?? { raw: lastText }, fallback: "empty_array" }, { status: 200 })
    }
    return NextResponse.json([], { status: 200 })
  }
