      ...
      <div className="container mx-auto px-4">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className={`fade-swap ${isAnimating ? "is-out" : ""}`}>
            <p className="inline-block bg-[#c89968] text-black uppercase text-sm md:text-base font-extrabold tracking-widest px-3 py-1 rounded-full mb-5">
              LANÇAMENTOS
            </p>
            ...
            {/* Mobile/tablet image positioned between textos */}
            <div className={`relative block w-full max-w-lg md:max-w-xl my-8 fade-swap lg:hidden ${isAnimating ? "is-out" : ""}`}>
              <div
                className="absolute -top-6 -right-6 w-24 h-24 border-4 border-[#c89968] animate-float z-10"
                style={{ animation: "float 9s ease-in-out infinite", willChange: "transform" }}
              />
              <div
                className="absolute -bottom-8 -left-10 w-20 h-20 border-2 border-[#c89968]/50 rounded-full"
            ...
            {current ? (
              <>
                ...
                <p className="text-gray-300 mb-6 leading-relaxed">
                  {[current.address, current.area].filter(Boolean).join(" • ")}
                </p>
              </>
            ) : (
            ...
            <div className="flex items-center gap-6">
              {current?.slug ? (
                <Link href={`/imovel/${current.slug}`}>
                  <Button className="bg-white hover:bg-gray-100 text-black px-8 py-6 font-bold tracking-wider">
                    VER MAIS
                  </Button>
                </Link>
              ) : (
          ...
          {/* Desktop image */}
          <div className={`relative hidden lg:block w-full max-w-2xl ml-auto fade-swap ${isAnimating ? "is-out" : ""}`}>
            <div
              className="absolute -top-10 -right-10 w-36 h-36 border-4 border-[#c89968] animate-float z-10"
              style={{ animation: "float 9s ease-in-out infinite", willChange: "transform" }}
            />
            <div
              className="absolute -bottom-12 -left-14 w-28 h-28 border-2 border-[#c89968]/50 rounded-full"
              style={{ animation: "float-3 11s ease-in-out infinite", willChange: "transform" }}
            />
            <div
              className="absolute -inset-3 border-2 border-[#c89968] rounded-2xl pointer-events-none"
              style={{ animation: "float-2 14s ease-in-out infinite", willChange: "transform" }}
            />
            <div className="relative w-full bg-gray-800 rounded-2xl overflow-hidden" style={{ aspectRatio: "1 / 1" }}>
              {(() => {
                const src = current?.image || "/placeholder.svg?height=600&width=800"
                function handleError(e: React.SyntheticEvent<HTMLImageElement, Event>) {
                  const el = e.currentTarget
